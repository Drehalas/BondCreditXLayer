...
type GuarantorWriteContract = GuarantorReadContract & {
  ...
  settleGuarantee(guaranteeId: string, x402PayloadHash: string): Promise<{ hash: string; wait(): Promise<unknown> }>;
  fundPool(overrides: { value: bigint }): Promise<{ hash: string; wait(): Promise<unknown> }>;
